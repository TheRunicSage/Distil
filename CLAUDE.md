...
[14] UI refresh phase 6c — `.lead` adoption across (app) shell pages (2026-05-10). Followed the dashboard / application/[id] migration pattern. Page hero subtitles converted from `text-lg text-muted-foreground` to `lead text-muted-foreground` on: `app/(app)/settings` (Admin tools subtitle), `app/(app)/upload`, `app/(app)/history`, `app/(app)/application/new`, plus admin pages `admin/usage`, `admin/logs`, `admin/telemetry`, `admin/users`. Admin pages keep their bespoke `text-4xl` h1s — those were intentionally distinct from `.heading-display` per Decision Log [14] earlier (admin headers are utilitarian, not editorial). Admin `users` count gets `.tabular` for the "{n} total" numeric. Admin status pills in usage tables NOT migrated — those use a per-status palette (`pill-success/queued/running/needs_input/error/stopped`) tuned for at-a-glance scanning that's distinct from the .pill primitive's variant set; left as-is per the Decision Log [13] note.

[14] UI refresh phase 6a — dashboard polish (2026-05-10). Hero subtitle and empty-state subtitles converted from `text-lg text-muted-foreground` to `lead text-muted-foreground` (the `.lead` primitive carries `text-lg sm:text-xl leading-relaxed` mobile-first scale; muted-foreground utility overrides .lead's text-text/90 default). `ChainCard` migrated from the bespoke pill recipe (`inline-flex items-center rounded-full border px-3 py-0.5 text-xs font-bold uppercase tracking-[0.06em] ${tone}`) to the design-system primitive (`pill ${tone-variant}`). `chainToneClass` in `lib/applications/chains.ts` updated to return pill variant suffixes (`pill-success`, `pill-warn`, etc.) instead of ad-hoc tailwind tuples — caller now writes `<span className={\`pill \${chainToneClass(tone)}\`}>`. Live-state chains get a `<span className="dot dot-pulse">` glyph inside the pill (the running-state cue from refresh vocab), inherits color from the surrounding pill via `currentColor`. ID monospace switched from `font-mono` utility to the new `.mono` primitive. Per-attempt list inside the disclosure also moved to `.pill` + `.mono .tabular`. No Tailwind utility removed wholesale — `font-mono` etc. still legal — but the call sites prefer the semantic primitives going forward.
...
[14] UI refresh kickoff — brief staged at `docs/ui-refresh/REFRESH.md` (2026-05-10). User dropped 7 Claude Design HTML exports (Sign in, Landing full flow, Master CV upload, Settings, Paused/needs-input, History grouped+scannable, Ready to send). Token-diff extracted via inline-style scrape (the artefact format inlines computed style on every element with the design tokens declared as CSS custom properties). Brand orange unchanged (`#e2613b`); semantic accents unchanged; Fraunces serif unchanged. Refresh changes: sans Outfit → Inter, mono added (JetBrains Mono), surface palette gains 5th elevation level + 3-level line system + 3-tier text scale, radii scaled larger, two new orange tokens (`orange-deep`, `orange-tint`), refresh primitives `.pill-{variants}`, `.dot`, `.btn-lg/sm`, `.mono`, `.tabular`, `.lead`, `.kbd`, `.chapter`, `.paper`. Polish thesis: **glow over hover** (orange-glow box-shadow as primary hover affordance for primary CTAs and interactive cards, applied site-wide; bg-shifts retained as secondary cue). Topbar redesign: 5-item UserMenu dropdown replaces standalone ThemeToggle + Settings gear (Email header · Theme toggle · FAQ · Settings · Sign out). New Application stays orange via `.btn-primary`, gains hover glow. Settings reachable only via UserMenu — no other nav surface (per user directive). DPs A/A/A/C/B/A. Migration phased: tokens → typography → primitives+glow → topbar/UserMenu → per-page polish. See REFRESH.md for the complete plan.
