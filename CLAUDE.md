...
[18] Cover letter closing — ban template phrases, require specific callback (2026-05-13). User-reported: cover letters always end with `"I would welcome the opportunity to discuss..."` — sounds overly formal and weird. Root cause traced to the prompt itself: §5.2 (Claude) / §6.2 (Flash) Paragraph 5 rule said `"Express willingness to discuss further"` — that instruction is template-shaped, which steers the model directly to the single most-overtrained recruiter-cliché in its training data. The §2.2 (Claude) / §7.2 (Flash) banned-phrase list covered openers and generic praise but had no closing-template bans.

Three coordinated edits in both prompts:

* **§2.2 / §7.2 banned-phrase list:** new "Phrase bans (cover letter closings)" subsection. Lists every overtrained closing template plus close cousins — `"I would welcome the opportunity / chance to discuss"`, `"I look forward to the opportunity / to discussing / to hearing from you"`, `"It would be a pleasure / a privilege to discuss"`, `"I would be delighted / honoured to"`, `"I am eager / keen to discuss how my [X] can [Y]"`, `"Please feel free to contact me"`, `"Thank you for considering my application"` / `"Thank you for your consideration"` as standalone sentences. Explicit note that `"Thanks for reading"` is a fine human variant. Hard ban on `cover_letter_content.paragraphs[4]`.
* **§5.2 / §6.2 Paragraph 5 rule:** rewritten from `"1 to 2 sentences. Thank the reader. Express willingness to discuss further."` to `"Specific callback + human voice; no templates."` Closing must reference one specific thing the body already established — a company project from research, a specific JD must-have addressed, a value or initiative connected to. Voice is conversational (contractions OK). Three to four shape-model examples included: `"Happy to talk through any of this in more detail, and to hear how the AP automation roadmap is shaping up under the REMS team."` / `"Will leave it there. Genuinely keen to talk through how I might fit, especially around the MRI rollout."` / `"Thanks for reading. Looking forward to learning more about the team's approach to the Wellington office expansion."` — each: one callback, no template phrases. Public-sector / cultural-acknowledgement variant: `"Nga mihi for reading. Keen to hear more about how AT is rolling out the fare simplification across South Auckland."`
* **§10 self-check items (Claude 44 / Flash 20):** scan `paragraphs[4]` for the banned template phrases listed in §2.2 / §7.2; rewrite with the specific-callback shape if any match. Second test: would a thoughtful human end a real cover letter to this specific company with this exact sentence? If the closing could be lifted verbatim into any cover letter for any role, it is a §2.5 "could anyone write this" failure — rewrite. Flash intro count bumped 19 → 20.

Root cause analysis (the broader pattern): prompts that describe an action ("Express willingness to discuss") in template-shaped language steer the model toward template phrases, especially for high-frequency closing/opening slots that have decades of trained data. The fix shape across the prompt — used here, used previously for openers, used for §0.2 "best-light principle" — is to (a) name and ban the templates explicitly, (b) describe what the slot does in terms of *function* not template ("specific callback + human voice"), and (c) provide shape-model examples for the model to anchor on.

What was *not* changed: the §5.2 paragraph structure (still 5 paragraphs, still Opening / Story 1 / Story 2 / Company Connection / Closing in order), word ranges (P5 still 40 to 60 words), the sign-off rules (per-country §8.2 row + `\n` break), the §10 paragraph-count check (item 23 / 11), the schema, the renderer.

Test path: re-submit any recent generation with the same JD. Expected: closing paragraph references a specific thing from the body (a project mentioned in P4 / a must-have addressed in P2 or P3 / a value mentioned in P1 or P4) and uses conversational voice — `"Happy to talk through"`, `"Would love to hear"`, `"Will leave it there"`, `"Thanks for reading"`, or similar shape; no `"I would welcome the opportunity"`, no `"I look forward to discussing"`, no `"It would be a pleasure"`. Re-audit if the same template phrases continue to surface across multiple generations despite the ban.
