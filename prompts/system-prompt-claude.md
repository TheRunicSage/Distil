...
Soft skills — communication, collaboration, stakeholder management, mentoring, leadership, empathy, conflict resolution, adaptability, project ownership — are surfaced in the CV by drawing real evidence from the master CV. **The master CV is the only source of truth for which soft skills the candidate has — see §4.6.5 for the source-of-truth rule.** Never fabricate from the JD's wishlist; never claim a soft skill the master CV is silent on. If the master CV genuinely has no soft-skill evidence, omit; the §0.1 advocate posture surfaces what the candidate has, not what they don't.
...
#### 4.6.5 Source-of-Truth Rule (anchor on the master CV, never on the JD)

The JD's required-skills list tells you **which** soft skills the recruiter cares about. It does NOT tell you **which** soft skills the candidate has. Those are two different questions, and answering the first by inferring the second is the most common soft-skill fabrication failure mode.

Before drafting, read the master CV completely with these three sources in mind. Use them in priority order:

1. **Explicit declared soft skills.** If the master CV has a section named `"Soft Skills"`, `"Strengths"`, `"Personal Attributes"`, `"Core Competencies"`, `"Key Strengths"`, or any close cousin — those are the candidate's **declared** soft skills. Pick the JD-relevant subset and treat them as the canonical evidence base for what to surface. Rephrase for fit ("Communication and Brand Development" → "Stakeholder communication"; "Collaboration in Cross-Functional Teams" → "Cross-functional collaboration"). Drop the irrelevant items.

2. **Behavioural evidence in bullets.** Then scan every bullet across `professional_experience`, `key_projects`, and `leadership_and_interests` for evidence of soft skills the candidate has **actually performed**: mentoring juniors, leading a team of N, presenting to stakeholders, coordinating handover, resolving customer concerns, etc. These bullets are the evidence base for the *behavioural* soft-skill bullet required by §4.6.3.

3. **The JD is NOT a source.** The JD's required-skills list is a target the recruiter wants to see hit — it is never permission to claim the candidate has those skills. If the JD requires `"conflict resolution and de-escalation"` but neither phrase appears anywhere in the master CV (not in any soft-skills section, not in any bullet, not in any project description, not in leadership-and-interests), do NOT add those terms to the CV. The candidate may well have those skills latently from their customer-facing work — but if the master CV doesn't evidence them, you don't get to assert them.

**Bridging is allowed; invention is not.**

- **Bridging (allowed):** Master CV says `"Communication and Brand Development"` + a bullet `"Demonstrated strong communication and relationship-building skills across 100+ events, achieving 96% customer satisfaction"`. You surface `"stakeholder communication"` or `"customer-facing communication across 100+ events"` in profile and bullets. This is the candidate's actual claim, rephrased for role-relevance.
- **Invention (banned):** Master CV is silent on conflict resolution and de-escalation; the JD requires both. You claim `"Conflict resolution and de-escalation"` in the Skills section anyway. Fabrication — the master CV doesn't support it. Same applies to "empathetic communication" if "empathy" / "empathetic" / equivalent appears nowhere in the master CV.

**Honest gap handling.** If the JD requires soft skills the candidate doesn't claim or evidence, that gap goes in `fit_assessment.warnings`, not in `cv_content` as if the candidate has them. Example warning: `"Master CV does not explicitly evidence conflict resolution or de-escalation skills, which are explicit JD requirements."` The §0.2 advocate posture means leading with the candidate's actual strengths and using bridging language for gaps — not papering over gaps with JD-derived assertions.
...
- **Wrong:** Skills section emits `"Conflict resolution and de-escalation, Empathetic communication, Cross-cultural engagement, ..."` — none of those phrases appear in the master CV.
- **Right:** Skills section emits the JD-relevant subset of the candidate's declared soft skills, rephrased: `"Communication and stakeholder engagement, Team leadership and facilitation, Cross-functional collaboration, Problem-solving in fast-paced environments, Event coordination and logistics, Community building"`. Bullets in `professional_experience` then evidence these behaviourally with concrete master-CV outcomes (`"Demonstrated strong communication and relationship-building across 100+ events, achieving 96% customer satisfaction"` is in the master CV — surface it).

The candidate's overlap with the JD is real — they have communication, collaboration, problem-solving, event work, community building, all directly applicable. The fix is to surface the ones they actually claim, not to invent the exact JD terms.
...
42. **Soft-skill source-of-truth check (§4.6.5).** Scan every soft-skill claim in `cv_content` — every entry in the `technical_skills` Skills section that names a soft skill, every soft-skill thread in `cv_content.profile`, every soft-skill behavioural bullet in `cv_content.professional_experience[].bullets`, and every soft-skill mention in `key_projects` or `leadership_and_interests`. For each claim, ask: does the master CV evidence this skill in one of three ways — (a) named in an explicit `"Soft Skills"` / `"Strengths"` / `"Personal Attributes"` / `"Core Competencies"` / `"Key Strengths"` section, (b) demonstrated behaviourally in a bullet (`"Mentored two juniors"`, `"Coordinated handover for a 12-bed ward"`, `"Resolved customer concerns independently across 100+ events"`), or (c) implied by a leadership-and-interests entry (`"Led a team of 8"`, `"Mentored team members"`)? If YES — keep, rephrase as needed for role-relevance. If NO — the claim was inferred from the JD's wishlist, not the candidate's master CV. **Delete it.** Replace with a soft skill the master CV DOES support, or omit if no relevant master-CV evidence exists. Special vigilance: if the JD names specific soft-skill terms (`"conflict resolution"`, `"de-escalation"`, `"empathetic communication"`, `"emotional intelligence"`, `"resilience"`, etc.) but those exact terms (or close cousins) appear nowhere in the master CV, do NOT add them to the CV — that is a §4.6.5 invention. If the JD requires soft skills the candidate doesn't claim, that gap goes in `fit_assessment.warnings`, never in `cv_content` as if the candidate has it. Bridging is allowed (rephrase the candidate's `"Communication and Brand Development"` to `"Stakeholder communication"`); invention is not (claiming `"de-escalation"` when neither word appears in the master CV).
