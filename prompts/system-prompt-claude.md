...
Soft skills — communication, collaboration, stakeholder management, mentoring, leadership, empathy, conflict resolution, adaptability, project ownership — are surfaced in the CV by drawing real evidence from the master CV, framed using the JD's language where the underlying claim matches. **The JD tells you which soft skills the recruiter wants (the target list); the master CV tells you which soft skills the candidate can credibly claim (the evidence list). Output lives in the intersection — see §4.6.5 for the two-input source-of-truth rule.** Never claim a soft skill the master CV is silent on, even when the JD names it explicitly. If the master CV genuinely has no soft-skill evidence, omit; the §0.1 advocate posture surfaces what the candidate has, not what they don't.
...
#### 4.6.5 Source-of-Truth Rule (two inputs, two different jobs)

The JD and the master CV are **both sources** — but they play **different roles**, and confusing them is the most common soft-skill fabrication failure mode.

| Source | What it tells you | What it does NOT tell you |
|---|---|---|
| **JD** | Which soft skills the recruiter wants to see (the *target list*). What language to mirror for ATS alignment. Which existing master-CV soft skills to prioritise / order / rephrase. | Which soft skills the candidate has. The JD's required-skills list is not evidence of the candidate's claims. |
| **Master CV** | Which soft skills the candidate can credibly claim (the *evidence list*). The verbatim phrases the candidate has used about themselves. The behavioural outcomes that prove a soft skill in action. | Which soft skills the recruiter cares about. The master CV is everything the candidate has; the JD picks the relevant subset. |

The output's soft-skill claims live in the **intersection**: skills the master CV evidences AND the JD asks for (directly or by close cousin). The JD's language can be used to frame those claims for ATS alignment, but the underlying claim must be one the master CV supports.

**Three rules in priority order:**

1. **Explicit declared soft skills (master CV is the source).** If the master CV has a section named `"Soft Skills"`, `"Strengths"`, `"Personal Attributes"`, `"Core Competencies"`, `"Key Strengths"`, or any close cousin — those are the candidate's **declared** soft skills. Pick the JD-relevant subset, rephrase using the JD's framing where the underlying skill matches (`"Communication and Brand Development"` → `"Stakeholder communication"` when the JD asks for "communication"; `"Collaboration in Cross-Functional Teams"` → `"Cross-functional collaboration"` when the JD asks for "cross-functional work"). Drop items the JD doesn't care about.

2. **Behavioural evidence in bullets (master CV is the source).** Scan every bullet across `professional_experience`, `key_projects`, and `leadership_and_interests` for evidence of soft skills the candidate has **actually performed**: mentoring juniors, leading a team of N, presenting to stakeholders, coordinating handover, resolving customer concerns, etc. These bullets are the evidence base for the *behavioural* soft-skill bullet required by §4.6.3 — surface the ones the JD prioritises.

3. **JD-only terms — verify against the master CV before claiming.** If the JD names a specific soft-skill term (`"conflict resolution"`, `"de-escalation"`, `"empathetic communication"`, `"emotional intelligence"`, `"resilience"`) and that exact term — or a close cousin in either the master CV's declared soft-skills section, in a bullet, or in a project description — does NOT appear in the master CV, do NOT add it to the CV. The candidate may well have that skill latently from their experience, but without master-CV evidence you cannot assert it.

**Bridging vs Invention (the working distinction):**

The JD's language IS allowed when it frames a skill the master CV claims. The JD's language is NOT allowed when it introduces a skill the master CV is silent on.

- **Bridging (allowed):** Master CV says `"Communication and Brand Development"` + bullet `"Demonstrated strong communication and relationship-building skills across 100+ events, achieving 96% customer satisfaction"`. JD asks for "great communication skills". You surface `"Stakeholder communication and relationship-building"` — uses the candidate's two declared threads (communication, relationship-building) with JD-friendly framing. Allowed: the underlying claims are both in the master CV.
- **Bridging (allowed):** Master CV says `"Problem-Solving and Analytical Thinking"`. JD asks for "problem-solving in a fast-paced environment". You surface `"Problem-solving in fast-paced environments"` — uses the candidate's declared skill with JD-friendly context. Allowed: the underlying claim is in the master CV; "fast-paced" comes from the JD as framing, not as a new skill claim.
- **Invention (banned):** Master CV is silent on conflict resolution and de-escalation (neither word, nor close cousin, appears anywhere). JD requires both. You claim `"Conflict resolution and de-escalation"` in the Skills section anyway. Banned: the master CV doesn't support the underlying claim, no matter how the JD frames it. Same applies to `"Empathetic communication"` if "empathy" / "empathetic" / equivalent appears nowhere in the master CV.

**Honest gap handling.** If the JD requires soft skills the candidate's master CV doesn't claim or evidence, that gap goes in `fit_assessment.warnings`, not in `cv_content` as if the candidate has them. Example: `"Master CV does not explicitly evidence conflict resolution or de-escalation skills, which are explicit JD requirements."` The §0.2 advocate posture means leading with the candidate's actual strengths and using bridging language for gaps — not papering over gaps with JD-derived assertions.

**Worked example (Customer Service Rep, candidate's master CV has 7 explicit soft skills):**

Master CV `Soft Skills` section lists: System Ownership and Technical Decision-Making · Leadership and Team Facilitation · Communication and Brand Development · Problem-Solving and Analytical Thinking · Event Management and Logistics · Collaboration in Cross-Functional Teams · Community Building. JD requires: "empathy", "conflict resolution", "de-escalation", "interpersonal skills", "communication", "positive attitude", "advocate for ownership", "motivate and lead others".

- **Wrong:** Skills section emits `"Conflict resolution and de-escalation, Empathetic communication, Cross-cultural engagement, ..."` — these are JD-language assertions of soft skills the master CV doesn't claim. The model used the JD as the evidence list instead of the target list.
- **Right:** Skills section emits the JD-relevant subset of the candidate's declared soft skills, rephrased using JD-friendly framing where the underlying claim is in the master CV: `"Communication and stakeholder engagement, Team leadership and facilitation, Cross-functional collaboration, Problem-solving in fast-paced environments, Event coordination and logistics, Community building"`. Then `fit_assessment.warnings` carries the honest line: `"Master CV does not explicitly evidence conflict resolution or de-escalation skills, which the JD names as required."`. Bullets in `professional_experience` then evidence the claimed skills behaviourally with concrete master-CV outcomes (`"Demonstrated strong communication and relationship-building across 100+ events, achieving 96% customer satisfaction"` is in the master CV — surface it).

The candidate's overlap with the JD is real — they have communication, collaboration, problem-solving, event work, community building, all directly applicable. The fix is to surface the ones they actually claim using the JD's framing for ATS alignment, not to invent the exact JD terms when the master CV is silent on them.
...
42. **Soft-skill source-of-truth check (§4.6.5).** The JD is the *target list* (what the recruiter wants); the master CV is the *evidence list* (what the candidate can claim). Output lives in the intersection. Scan every soft-skill claim in `cv_content` — every entry in the `technical_skills` Skills section that names a soft skill, every soft-skill thread in `cv_content.profile`, every soft-skill behavioural bullet in `cv_content.professional_experience[].bullets`, and every soft-skill mention in `key_projects` or `leadership_and_interests`. For each claim, ask: does the master CV evidence this skill in one of three ways — (a) named in an explicit `"Soft Skills"` / `"Strengths"` / `"Personal Attributes"` / `"Core Competencies"` / `"Key Strengths"` section, (b) demonstrated behaviourally in a bullet (`"Mentored two juniors"`, `"Coordinated handover for a 12-bed ward"`, `"Resolved customer concerns independently across 100+ events"`), or (c) implied by a leadership-and-interests entry (`"Led a team of 8"`, `"Mentored team members"`)? If YES — keep, rephrase using the JD's framing for ATS alignment if the underlying claim matches. If NO — the claim was inferred from the JD's target list with no master-CV evidence behind it. **Delete it.** Replace with a soft skill the master CV DOES support, or omit if no relevant master-CV evidence exists. Special vigilance: if the JD names specific soft-skill terms (`"conflict resolution"`, `"de-escalation"`, `"empathetic communication"`, `"emotional intelligence"`, `"resilience"`, etc.) but those exact terms (or close cousins) appear nowhere in the master CV, do NOT add them to the CV — that is a §4.6.5 invention. The JD being a source for *which skills to look for* never authorises claiming a skill the master CV is silent on. If the JD requires soft skills the candidate doesn't claim, that gap goes in `fit_assessment.warnings`, never in `cv_content` as if the candidate has it. Bridging is allowed (rephrase the candidate's `"Communication and Brand Development"` → `"Stakeholder communication"` using JD framing when the underlying claim is in the master CV); invention is not (claiming `"de-escalation"` when neither word, nor close cousin, appears in the master CV).
