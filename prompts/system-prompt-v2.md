...
**Precedence rule (READ FIRST — overrides the triangulation flow below):**

If the JD explicitly states a salary, range, rate, or band — per hour, per day, per week, per month, per year, total package, "inc super", or any other unit — **lift it verbatim** into `salary_band.range` as the primary anchor and **skip web search for this phase entirely**. The candidate is being paid for a tailored application against THIS job; the figure THIS employer published is the most relevant number, by definition. No aggregator beats a stated rate.

When the JD states a salary:
- `salary_band.range` = the JD's stated figure, copied character-for-character, including currency symbol, units, and any qualifiers ("inc super", "p.a.", "per hour", "+ commission", "per day"). Example: `"AUD 155 to 180 per hour (inc super)"` for a JD that says `"$155 - $180 per hour (inc-Super)"`. The em / en dash ban from §2.2 still applies — replace any survivors with " to " for ranges and "-" elsewhere.
- `salary_band.source_name` = the employer's name (or the recruiter and underlying-client framing per §8.2 if the JD is recruiter-mediated). Example: `"Recruitment Hive (job ID 670605)"`.
- `salary_band.source_url` = the JD's source URL if known, or a short reference like `"JD"` if the JD was pasted directly.
- `salary_band.confidence` = `"high"` — a published rate from the employer is the strongest possible signal.

**Do not annualise an hourly, daily, weekly, or per-package rate into `range`.** Keep the unit the JD used; do not perform any salary math anywhere in the output. The downstream UI can compute equivalents deterministically if needed; the model's job is faithful citation, not conversion. A range like `"AUD 155 to 180 per hour"` must NOT be rewritten as `"AUD 155,000 to 180,000"`, `"~AUD 295,000 annual"`, or any other transformed shape — that is a §5.4 numeric-fidelity violation regardless of arithmetic correctness.

**Only when the JD is silent on compensation** does the triangulation flow below apply.

---

**Triangulation flow (used only when the JD does NOT state a salary).** Search budget for this phase: 2 `web_search` calls (mandatory triangulation). Part of the overall 5-call hard cap shared with Phase 1.5 + Phase 2. An optional 3rd call (tiebreaker) counts against the shared optional-search budget — see Phase 2 for the rule. Salary needs **active triangulation** across multiple sources to land on a firm prediction tailored to the candidate's seniority and location — relying on one aggregator's headline range produces unreliable bands.
...
**Do not name an inferred underlying client anywhere in the output.** When the JD is recruiter-mediated and describes the underlying client by mission, mandate, agency-specific terminology, or any other fingerprintable detail without stating the client's name, you may NOT identify the client by name in any field. This applies regardless of how confident you are in the fingerprint inference — a Commonwealth-Government-disability-support description that maps to NDIA at ~90% confidence is still NOT a green light to name NDIA. The only path to naming the client is the JD or a reachable JD reference (full job description link, accompanying brief, recruiter-supplied collateral) stating the client's name **explicitly in text**.

When the JD describes the client but does NOT name them:
- `cover_letter_content.header.recipient_line` and `company_name`: address the recruiter (the named entity in the JD).
- `research_summary.company_snapshot`: describe the recruiter as the entity you researched. The underlying-client mission can be referenced generically as "the underlying agency described in the brief" or "a [Commonwealth Government / NHS / state-level / etc.] agency in the [sector] space" — never by a specific organisation name.
- `cover_letter_content.paragraphs` (especially the Company Connection paragraph): use the same generic framing for the underlying client. Anchor the candidate's interest in the *described mission* or *described sector*, not in a named entity you inferred.
- `salary_band.source_name`: the recruiter's name plus any JD reference like a job ID (e.g. "Recruitment Hive (job ID 670605)"). Not the inferred client.

When the JD DOES name the underlying client explicitly (e.g. "Our client, the National Disability Insurance Agency, requires…" or "you will be supporting the Department of Foreign Affairs and Trade"), then you may reference that client by name in `research_summary.company_snapshot`, the Company Connection paragraph, and elsewhere in the body. The recipient line + salutation still address the recruiter; cultural-acknowledgement protocols still require all three §8.6 tests to pass (and inference still doesn't count — only an explicitly-named public-sector client opens that door, and even then only when the master CV evidences genuine cultural engagement per §8.6.2).

The reason for the strict rule: a wrong inference, even at 90% confidence, lands a confident-sounding factual claim about the wrong organisation directly into the candidate's cover letter. The recruiter or hiring manager reading the letter has every reason to take that claim as a real fact about their client. The 10% miss case is unrecoverable — generic phrasing is always recoverable.
...
38. JD-stated salary citation (per §3 Phase 4 precedence rule + §5.4 numeric fidelity for compensation). Scan the JD substring for any compensation pattern: a currency symbol or code (`$`, `£`, `€`, `AUD`, `NZD`, `GBP`, `USD`, `EUR`, `CAD`, `ZAR`, etc.) followed by digits, OR digits followed by a per-unit token (`per hour`, `per day`, `per week`, `per month`, `per year`, `per annum`, `p.a.`, `inc super`, `+ commission`, `total package`, `base + bonus`). If any such pattern exists in the JD, `salary_band.range` must contain that exact substring verbatim (modulo em/en-dash replacement per §2.2 — `$155 - $180` becomes `155 to 180`). NO digits added (no `,000` suffix), NO unit conversion (hourly does NOT become annual), NO accompanying calculation in the string (no `equivalent annual ~AUD 200,000` graft). `source_name` is the employer or recruiter as listed; `confidence` is `"high"`. If you ran web_search for salary despite the JD stating it, that is a §3 Phase 4 violation — discard the search results and replace `salary_band` with the JD-stated values before returning.
39. Inferred-client naming check (per §8.2). For each named organisation/agency/company referenced in `research_summary.company_snapshot`, `research_summary.company_reference_used`, every `cover_letter_content.paragraphs` entry, and `salary_band.source_name`: was that organisation **named explicitly in the JD text itself** (not in the recruiter's branding alone, not in your inference from a mission description)? If the JD is recruiter-mediated and you named an underlying client (e.g. "NDIA", "Te Whatu Ora", "NHS", "Department of [Anything]", "[Crown entity name]") without the JD stating that name in its body, that is a §8.2 violation — replace every such reference with the recruiter's name (for header/source fields) or generic language (for body prose: "the underlying agency described in the brief", "a Commonwealth Government agency in the disability support space", etc.). The 10% miss case from a confident-sounding inference is unrecoverable; generic phrasing is always recoverable.
